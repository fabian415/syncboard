...
const RUNTIME_JS = [
  ...
  '    return sections;',
  '  }',
  '  // Mirrors STICKY_HEADING_RE / mergeStickySections in utils/slides.js:',
  '  // 「補充說明 (Why)」 explains the 「核心重點」 bullets and their',
  '  // screenshots, so the three are folded into one indivisible unit.',
  '  var STICKY_HEADING_RE = /補充說明|[(（]\\s*why\\s*[)）]/i;',
  ...
  '        prev.blocks = prev.blocks.concat([section.heading], section.blocks);',
  '        prev.atomic = true;',
  '        continue;',
  '      }',
  '      merged.push(section);',
  ...
  '      var rest = titleNode ? nodes.slice(1) : nodes;',
  // The leading Owner heading is page furniture (the viewer hoists it into the
  // topbar badge, and <h1> is display:none on a slide), so it belongs to every
  // page's header rather than being a heading-only section of its own — which
  // would otherwise flush a page that renders completely blank. Mirrors
  // client/src/utils/slides.js.
  '      function isOwnerHeading(el) { var t = (el.textContent || "").trim(); return t.indexOf("Owner：") === 0 || t.indexOf("Owner:") === 0; }',
  '      var ownerNode = rest[0] && rest[0].tagName === "H2" && isOwnerHeading(rest[0]) ? rest[0] : null;',
  '      var headerNodes = [titleNode, ownerNode].filter(Boolean);',
  '      var sections = mergeStickySections(groupIntoSections(ownerNode ? rest.slice(1) : rest));',
  '      var pages = [];',
  '      var pageNodes = [];',
  ...
  '        pageNodes = headerNodes.map(function (n) {',
  '          return isContinuation && n === titleNode ? cloneWithContinuedSuffix(n) : n.cloneNode(true);',
  '        });',
  ...
  '        var hasOtherContent = pageNodes.length > headerNodes.length;',
  '        if (hasOtherContent) {',
  '          startNewPage(true);',
  ...
  '        }',
  // The 核心重點 + 圖片 + 補充說明 unit is never split: it keeps its page at
  // full size and the slide scrolls if it's taller than the stage.
  '        if (section.atomic) {',
  ...
  '',
  '  function repaginate() {',
  // Same fullscreen guard as PresentationModal.runAutoPaginate: entering
  // fullscreen on an embedded <video> fires a resize, and re-rendering the
  // slide would pull the fullscreen element out of the document, which makes
  // the browser exit fullscreen immediately.
  ...
  // A fullscreen <video> owns the keyboard: Esc leaves fullscreen, and arrows
  // must not flip the page out from under it.
  '    if (document.fullscreenElement) return;',
  '    if (e.key === "Escape") {',
  ...
  // Pick the layout pass back up once fullscreen closes and the viewport is
  // back to its normal size.
  '  document.addEventListener("fullscreenchange", function () {',
  '    if (!document.fullscreenElement) scheduleRepaginate();',
  '  });',
  '',
  '  repaginate();',
  '})();',
...
// h1, h2, ul/ol/li (nested), strong, a, img, video, span.badge-*, and
// div.card / div.card.warning / div.kms-link / div.grid / div.image-row /
// div.video-block — so the converter only needs to understand that
// vocabulary, not arbitrary HTML.

function headingMark(level) {
  return '#'.repeat(Math.min(Math.max(level, 1), 6));
}

// <img>/<video> carry everything in their attributes and have no child nodes,
// so they can't go through convertInline (which walks childNodes) when they
// are the element being converted rather than one of its children.
function convertMedia(el) {
  const src = el.getAttribute('src') || '';
  if (el.tagName === 'VIDEO') return `!video[${el.getAttribute('title') || ''}](${src})`;
...
function convertBlocks(container, blocks, headingBase) {
  Array.from(container.children).forEach((child) => {
    ...
    if (tag === 'h1') {
      ...
      blocks.push(convertMedia(child));
    } else if (tag === 'div') {
...
function convertDiv(div, blocks, headingBase) {
  ...
  if (cls.includes('image-row')) {
    const imgs = Array.from(div.children)
      ...
      .map((img) => convertMedia(img));
    if (imgs.length) blocks.push(imgs.join('\n'));
    return;
  }
  if (cls.includes('video-block')) {
    const videos = Array.from(div.children)
      .filter((c) => c.tagName === 'VIDEO')
      .map((video) => convertMedia(video));
