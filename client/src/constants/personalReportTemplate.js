...
| 圖片跳轉連結 | \`[圖一](#1)\`、\`[圖二](#2)\` | 點擊直接放大顯示對應的第 N 張圖片，不是外部連結 |

圖片與影片請先到「整體進度上傳 → Deep Dive」上傳，再用卡片上的「複製連結」／「複製 Markdown 語法」取得網址；語法要獨立成一行。

想讓一段文字「點下去直接跳到某張圖片放大」，就在文字裡加 \`[圖一](#1)\`、\`[圖二](#2)\`……，數字 N 是這張圖片在本段落中依出現順序的第幾張（從 1 開始算，不用管它被排進哪一個圖片區塊）。放大檢視時 Esc 可離開、左右鍵仍可切換到同頁其他張圖片。
-->`;
}
