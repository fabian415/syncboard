...
7. 原文中若有獨立成行的圖片語法 \`![替代文字](網址)\`（該行本身只有圖片語法），要轉換成 <img src="網址" alt="替代文字">，放在其所屬 📦 子項目「本週進度」清單之後、下一個 📦 子項目（或結尾的「⚠️ 風險與阻礙」「💡 需要的決策或協助」）之前。**不論張數，所有 <img> 一律要包在 <div class="image-row"> 裡面**：同一位置有 2 張以上連續的圖片就包在同一個 <div class="image-row"> 裡讓它們並排顯示；只有 1 張時也一樣要包，輸出成 <div class="image-row"><img src="..." alt="..."></div>，**絕對不能**讓 <img> 單獨出現在 <div class="image-row"> 之外。不能省略任何一張圖片。
8. ${VIDEO_SYNTAX_RULE}
