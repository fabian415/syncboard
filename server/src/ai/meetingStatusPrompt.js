...
輸入格式固定為：標題「# 📋 Meeting Overall Status｜日期」+ 主持人/與會人員一行、"## 1. Follow-up" 追蹤事項清單（每項含 Owner／狀態，可能有 Description／Result，這兩欄底下可能各自帶一層或多層縮排的子項目，代表細項或補充要點；整體可能有補充說明）、"## 2. Product Overall Status" 底下依 "### 🌐 Product名稱" 分段，每個 Product 底下可能有多個 "#### 📦 子項目名稱"（階段＋本週進度條列），結尾兩行「⚠️ 風險與阻礙」「💡 需要的決策或協助」。Follow-up 的子項目、以及 Product 本週進度條列底下更深縮排的子項目，都可能整條就是圖片語法 \`![替代文字](網址)\`，代表附加的截圖或附件；「本週進度」清單之後也可能直接接著獨立成行的圖片語法。

嚴格分頁規則：
1. 第 1 頁＝標題＋主持人/與會人員＋<h2>1. Follow-up</h2>＋整個 Follow-up 區塊（含補充說明，若補充說明是「無」則省略該行）。Follow-up 每個項目各自輸出一組 <h2><ul>：<h2> 固定格式為「<strong>事項名稱</strong>（Owner：xxx）狀態徽章」，狀態徽章一律放在整行最後面（在（Owner：xxx）之後），**每個項目都必須有這個徽章**：狀態文字能對應到下方轉換表就用對應顏色的 <span class="badge badge-xxx">；對應不到（自訂階段描述，如「驗證中」「Ongoing」）就用 <span class="badge badge-neutral">原文字（含 emoji）</span>，不要把狀態文字留在括號內當純文字。<h2> 底下緊接 <ul>，Description、Result 各自輸出為一個子項目 <li>（依原文先後順序，文字固定用 <strong>Description</strong>、<strong>Result</strong>，只輸出原文中有出現的欄位），該 <li> 底下再接一層巢狀 <ul>，把該欄位原文的子項目依縮排層級原樣對應轉成 <li>（子項目 → 次項目 → 更下一層…，原文縮排幾層就巢狀幾層 <ul>），逐條列出、不可省略、合併或攤平成同一層。若某條子項目原文開頭帶狀態標籤（見下方轉換表），轉換成對應 <span class="badge ...">放在該 <li> 最前面；沒帶標籤的子項目就只輸出文字，不要加 badge-neutral。Description、Result 底下若有子項目本身就是圖片語法 \`![替代文字](網址)\`（該行只有圖片語法，沒有其他文字），**絕對不能**輸出成 <li>、也不能放進任何 <ul> 或 <li> 裡面；要把該追蹤項目底下出現的所有這類圖片（依原文先後順序）收集起來，改在該項目 <h2>...</h2><ul>...</ul> 結構之後（即該事項最外層 </ul> 結束、下一個事項 <h2> 之前）另外輸出一個區塊，**不論張數，所有 <img> 一律要包在 <div class="image-row"> 裡面**：2 張以上圖片包在同一個 <div class="image-row"> 裡並排顯示；只有 1 張時也一樣要包，輸出成 <div class="image-row"><img src="..." alt="..."></div>，**絕對不能**讓 <img> 單獨出現在 <div class="image-row"> 之外。不能省略任何一張圖片。
2. 之後**每一個 "### 🌐 Product 名稱" 區塊各自輸出一頁**（不可把多個 Product 合併在同一頁，也不可把同一個 Product 拆成多頁），該頁 <h1> 之後緊接著輸出一個 <h2>Owner：xxx</h2>（使用該 Product 底下唯一的 "**Owner**" 值，只在頁首出現一次，不可在每個子項目重複），接著保留該 Product 底下所有 "#### 📦 子項目"，每個子項目用一個 <h2> 標出「子項目名稱（階段：xxx）」。「本週進度」條列項目底下若有更深一層縮排的純文字子項目（沒有狀態標籤），要包成巢狀 <ul><li>...</li></ul> 放在該狀態項目的 <li> 內部，忠於原文、不省略。「本週進度」清單之後、下一個 📦 子項目（或結尾的「⚠️ 風險與阻礙」「💡 需要的決策或協助」）之前，若有獨立成行的圖片語法 \`![替代文字](網址)\`，要轉成 <img src="網址" alt="替代文字">，且**不論張數，所有 <img> 一律要包在 <div class="image-row"> 裡面**：2 張以上連續的圖片包在同一個 <div class="image-row"> 裡並排顯示；只有 1 張時也一樣要包，輸出成 <div class="image-row"><img src="..." alt="..."></div>，**絕對不能**讓 <img> 單獨出現在 <div class="image-row"> 之外。不能省略任何一張圖片。
